...
namespace GroGroup.Controllers
{
    ...
    public class UtilityController : Controller
    {
        ...
        private DataSet ConvertSlsFileToDataset(string fileName)
        {
            ...
            excelConnectionString = "Provider=Microsoft.ACE.OLEDB.16.0;Data Source=" + _Path + ";Extended Properties=\"text;HDR=No;FMT=CsvDelimited;\"";
        ...
        private DataSet ConvertPDSExcelFileToDataset(string fileName)
        {
            ...
            excelConnectionString = "Provider=Microsoft.ACE.OLEDB.16.0;Data Source=" + _Path + ";Extended Properties=Excel 12.0;";
            OleDbConnection excelConnection = new OleDbConnection(excelConnectionString);
            excelConnection.Open();
                    ...
                    for (int i = 0; i < drUpd.Length; i++)
                    {
                        //if (drUpd[i]["UPC number  (Normally 12 Digits)"] != DBNull.Value && drUpd[i]["UPC number  (Normally 12 Digits)"].ToString().Trim() != "")
                        {
                            ...
                            if (drUpd[i]["Distributor Price / Ea.   Required"] != DBNull.Value && drUpd[i]["Distributor Price / Ea.   Required"].ToString() != "")
                                dr["dist_ea"] = drUpd[i]["Distributor Price / Ea.   Required"].ToString();
...
*/
                            ...
                            if (PDSDS.Tables["PDS"].Columns.Contains("Product Description (Overflow from Column I) "))
                            {
                                if (drUpd[i]["Product Description (Overflow from Column I) "] != DBNull.Value && drUpd[i]["Product Description (Overflow from Column I) "].ToString() != "")
                                    dr["f01J_prodcDesc2"] = drUpd[i]["Product Description (Overflow from Column I) "].ToString();
                            }
